...
public sealed class CommandManager : AvaloniaObject
{
    ...
    static CommandManager()
    {
        ...
        Control.UnloadedEvent.AddClassHandler<Control>(UnloadedEventHandler);
    ...
    private static void UnloadedEventHandler(Control targetElement, RoutedEventArgs args)
    {
        if (args.Source is IInputElement el)
        {
            if (FocusedElement == el)
            {
                s_inputElement.SetTarget(null);
                Current.RaiseRequerySuggested();
            }
        }
    }
