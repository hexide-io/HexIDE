...
public class MDIHost : ItemsControl
{
    ...
    {
        const double iconWidth = 160;
        const double iconHeight = 24;
        const double gap = 2;

        var hostWidth = Bounds.Width;
        var hostHeight = Bounds.Height;
        var iconsPerRow = Math.Max(1, (int)((hostWidth + gap) / (iconWidth + gap)));
        ...
        {
            var container = ContainerFromIndex(i);
            if (container == null || MDIHostPanel.GetWindowState(container) != WindowState.Minimized)
                continue;

            var x = col * (iconWidth + gap);
            var y = hostHeight - iconHeight - row * (iconHeight + gap);
            MDIHostPanel.SetWindowLocation(container, new Point(x, y));
