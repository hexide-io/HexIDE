# protocol-inspector Specification

## Purpose
Define the record HexIDE keeps of its conversations with language servers: what is captured, what is
retained, what leaves the machine, and how a person reads it.

**A language feature that silently does nothing is the hardest kind of defect this project has**, because
every explanation looks identical from the editor. The request was never sent; it was sent and never
answered; the server answered and the client discarded it; the server never claimed the capability; the
server died. A diagnostics list cannot separate those, and neither can a log — the question is about
messages, and until this existed nothing in the IDE could show one.

**The record is therefore always on and content-free by default.** A conversation cannot be captured
retrospectively, and the thing a developer wants to look at is almost always something they noticed after
it happened; a tool that has to be armed first answers only the questions its user already knew to ask.
Envelopes cost a few dozen bytes each and disclose nothing, which is what makes always-on defensible.
Bodies are the opposite trade and are kept only when asked for — except each connection's handshake, which
cannot be captured after the fact and is always kept.

**Five kinds of entry are not messages at all, and they are the point.** What the client declined to send
and why; what the server offered that nothing took up; what this transport structurally cannot observe;
the process's own standard error and exit code; and a frame that could not be decoded. Each is the answer
to a question a wire trace alone cannot settle, and each was invisible before.

This replaced an interposed debug proxy. Comparing the two is what found three places where the record
claimed more than it held — standard error, the exit code, and an undecodable frame — and none of them had
a failing test. That is the standing hazard here: **a record that names the wrong cause for an absence is
worse than one that stays silent**, because the whole value of the thing is that its answers can be
trusted.
