...
|---|---------|-----------|--------|-----|
| C1 | ~~Nested `Begin` blocks flattened — menu trees and container membership destroyed~~ **FIXED** (#83 menus, #84 containers) | The loader records a menu tree on the parent's SubItems and a control tree on `ComponentInstance.Container`/`ContainedControls`; the writer walks both back out through one children-of helper | was 12/22; now 0 held for nesting | done |
...
| C4 | `ScaleWidth`/`ScaleHeight` discarded on load, fabricated from the client rect in twips | **Half fixed by #84**: a CONTAINER's Scale* are now preserved verbatim, so a PictureBox no longer declares a user-defined scale with no scale. The ROOT's are still recomputed from its own Width/Height, which is wrong for any non-twips `ScaleMode` — tracked as [#92](https://github.com/hexide-io/HexIDE/issues/92) | 3 root-level (Colorful Control.ctl **15×**, About Dialog +45%, Log in Dialog) | **M** (verbatim) / **L** (modelled) |
...
| C7 | Binary props of unknown controls dropped **and** blob destroyed on `.frx` rewrite | **Contained, not fixed**: the property is still dropped, but the loader now flags it and the form is refused rather than saved (#89), and the save path never rewrites a companion it cannot reproduce. Splash Screen is one of the six forms this holds read-only | Splash Screen and 5 others | **M** to actually preserve the blob |
...
| C9 | ~~Unknown controls re-parented to the form root~~ **FIXED** (#84) | The preserved block is recorded on the component it was read from, indented to its real load depth, and re-emitted at the ordinal it held among that component's children — so `imgLogo` stays inside Frame1 | was 2, 3 more predicted | done |
...
| C11 | ~~`ReloadFormFromDisk` never syncs `UnknownChildSubtreeTexts`~~ **FIXED** (#84) | The preserved blocks moved onto the components themselves, so `UpdateComponents` carries them; the reload also adopts the fresh parse's whole fidelity verdict via `AdoptFidelityState` rather than only Code and Components | IDE path | done |
