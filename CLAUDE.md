...
3. **Translate every new key into all shipped packs in the same change — the build enforces it.**
   ...
   commits. **Only a pull request from a fork is exempt.** Outside contributors add English only
   (CONTRIBUTING.md), and on their PRs `build.yml` runs that test as a non-blocking warning. The maintainer
   translates before merging, and the push to `main` runs the test as a blocking check. That relaxation
   is for contributors. Work in this tree still translates in the same change. The moment you add a `Str.*` key to `en`, add its translation to each shipped
...
- **`docs/command-line.md`** — every command-line option HexIDE accepts, what each does, and which two are
  inert in a distributed build. The only user-facing account; keep it in step with
  `HexIDE.Desktop/ServerOptions.cs`, which is the parser it describes.
...
- **[`docs/issue-labels.md`](docs/issue-labels.md)** — what each issue label means and which one wins when two look plausible. Four axes: one `type`, one `size`, up to three `area`, and zero or more `state`. **Read it before labelling an issue**, and treat its rules literally — each one was written to settle a case two careful readers decided differently. It is *not* a living document in the guarded sense: its source of truth is the label set on GitHub, which no test here can reach. How the set was arrived at is in [`docs/archive/issue-labels-decided.md`](docs/archive/issue-labels-decided.md).
