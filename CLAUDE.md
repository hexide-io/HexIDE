...
- **Avalonia 12.0.4 / Dock 12.0.0.2** — the project is on Avalonia 12. `Classic.Avalonia.Theme 11.3.0.3` is kept as a **controls-only** dependency (provides `ClassicBorderDecorator`, `ClassicBorderStyle`). `<ClassicTheme />` is **NOT loaded** — it crashes at runtime with Avalonia 12 (`MissingMethodException: DynamicResourceExtension.ProvideValue`). Do not add `<ClassicTheme />` back. The `Classic.Avalonia.Theme.Dock`, `.ColorPicker`, and `.DataGrid` sub-packages have been removed. `Avalonia.Themes.Simple 12.0.4` is the base theme (`<SimpleTheme />` in App.axaml).
- **`Classic.CommonControls.Avalonia 11.3.0.3` controls** (`ToolBar`, `ToolBarButton`, `RebarHandle`, `ListView`, `ListViewItem`) are **NOT used** — they crash at runtime with `TypeLoadException: PseudolassesExtensions` (removed in Avalonia 12). Use standard Avalonia `StackPanel`/`Button`/`ToggleButton`/`ListBox` instead. `SystemColors` static resource keys from that assembly are still used for color lookups and do not crash.
