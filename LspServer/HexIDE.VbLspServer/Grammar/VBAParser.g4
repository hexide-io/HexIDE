...
// '+' on the inter-statement separator so blank lines (>1 end-of-statement) inside a
// For/For Each body are absorbed — matching `block` above. Without it, a blank line between
// two statements in a loop body ended the block early and orphaned the rest before NEXT,
// producing cascading false "syntax error" diagnostics on valid code.
unterminatedBlock : blockStmt (individualNonEOFEndOfStatement+ blockStmt)*;
